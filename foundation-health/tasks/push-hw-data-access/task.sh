...
healthwatch_product_guid=$(om -k -t "$OPSMAN_URL" -u "$OPSMAN_USERNAME" -p "$OPSMAN_PASSWORD" curl -p /api/v0/deployed/products | jq -r '.[] | select(.installation_name | contains("p-healthwatch")) | .guid')
